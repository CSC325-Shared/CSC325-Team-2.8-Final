...
client.on(Events.GuildMemberUpdate, (oldMember, newMember) => {
	if (oldMember.roles.cache.size < newMember.roles.cache.size) {
		...
		database.writeToLogChannel(`Role <@&${addedRole.id}> added to user **<@${newMember.id}> (${newMember.user.tag})**`);
	}
	else if (oldMember.roles.cache.size > newMember.roles.cache.size) {
		// Find the removed role
		const removedRole = oldMember.roles.cache.find(role => !newMember.roles.cache.has(role.id));
		// Log the role name and id
		database.writeToLogChannel(`Role <@&${removedRole.id}> removed from user **<@${newMember.id}> (${newMember.user.tag})**`);
	}
});
