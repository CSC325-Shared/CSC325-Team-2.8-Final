...
module.exports = {
	...
	async execute(interaction, database) {
		...
		if (dbv) {
			...
			const rolesList = [...interaction.guild.roles.cache.values()];
			...
			new Promise((resolve) => {
				...
				if (cohabitate) {
					...
					announcementsChannel.permissionOverwrites.edit(studentRoleID, { ViewChannel: true,
						...
						CreatePublicThreads: false }).then(() => {
					...
					database.getCoursesWithCategory(cohabitate.id).then(courses => {
						...
						cohabitate.setName(curCoursesTitle).then(() => {
							announcementsChannel.setName('announcements-' + curCoursesNums).then(() => {
				...
				database.saveCourse(courseObj).then(() => {
...
function findPosition(name, array) {
	...
	const largestNum = parseInt(array[0].name);
	const smallestNum = parseInt(array[array.length - 1].name);
	...
	for (let i = 0; i < array.length; ++i) {
		const currentNumber = parseInt(array[i].name);
