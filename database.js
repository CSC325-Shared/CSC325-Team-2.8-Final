...
class Database {
	...
	setup() {
		...
		initDB(this.db, dbEditTable).then(cat => {
			...
			const alsoDB = this.db; // Assign a local variable to db, as 'this' refers to something else in callback method
